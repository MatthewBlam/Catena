...
function getInitialDark(): boolean {
  const stored = localStorage.getItem("catena-theme");
...
function App(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    localStorage.setItem("catena-theme", dark ? "dark" : "light");
  }, [dark]);
      ...
      <div className="w-54 shrink-0 p-3 flex flex-col">
        <nav className="h-full bg-sidebar rounded-xl p-2 flex flex-col border border-border">
          ...
          <div className="flex-1 min-h-0 flex flex-col">
            ...
            {recents.length > 0 && (
              <>
                <div className="mt-4 px-3 pb-1 text-xs font-medium text-sidebar-foreground">Recents</div>
          ...
          <div className="mt-6 flex items-center justify-center gap-1.5 pb-2">
            <svg viewBox="0 0 5000 5000" fill="currentColor" className="ml-[-5.5px] size-8 shrink-0 text-sidebar-primary">
            ...
            <span className="text-2xl font-black text-sidebar-primary">CATENA</span>
          </div>
        </nav>
      </div>
          ...
          {visited.has("settings") && (
            <ErrorBoundary resetKeys={[page]}>
              <div style={{ display: page === "settings" ? undefined : "none" }}>
                <SettingsPage visible={page === "settings"} dark={dark} onToggleTheme={handleToggleTheme} onProviderReset={handleProviderReset} />
              </div>
            </ErrorBoundary>
          )}
        </main>
